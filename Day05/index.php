        ...
        for ($j = 0; $j < sizeof($level); $j++) {
            ...
            if ($seedRanges[$i][0] >= $level[$j][1] && $seedRanges[$i][0] < $level[$j][1] + $level[$j][2]) {
                ...
                if ($seedRanges[$i][1] >= $level[$j][1] + $level[$j][2]) {
                    ...
                    // print_r($seedRanges[$i]);
                    // echo '<br>';
                    array_push($newSeedRanges, convert($seedRanges[$i][0], $level[$j]), convert($level[$j][1] + $level[$j][2] - 1, $level[$j]));
                    $modified = true;
                ...
                if ($seedRanges[$i][0] < $level[$j][1]) {
                    ...
                    // print_r($seedRanges[$i]);
                    // echo '<br>';
                    array_push($newSeedRanges, convert($level[$j][1], $level[$j]), convert($seedRanges[$i][1], $level[$j]));
                    $modified = true;
                }
                // Si la range supérieure ET la inférieure sont exclues
            } else if ($seedRanges[$i][0] < $level[$j][1] && $seedRanges[$i][1] >= $level[$j][1] + $level[$j][2]) {
                array_push($newSeedRanges, convert($level[$j][1], $level[$j]), convert($level[$j][1] + $level[$j][2] - 1, $level[$j]));
                $modified = true;
            }
        }
        // if ($modified === false) {
...
// for ($k = 0; $k < sizeof($seeds); $k++) {
//     for ($i = 1; $i < sizeof($data); $i++) {
//         for ($j = 0; $j < sizeof($data[$i][1]); $j++) {
//             $seed2 = convert($seeds[$k], $data[$i][1][$j]);
//             if ($seed2 != $seeds[$k]) {
//                 $seeds[$k] = $seed2;
//                 break;
//             }
